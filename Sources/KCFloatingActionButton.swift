...
//    private var overlayLayer: CAShapeLayer = CAShapeLayer()
    ...
    */
     ...
     */
    public func addItem(title: String, handler: @escaping ((KCFloatingActionButtonItem) -> Void)) -> KCFloatingActionButtonItem {
        let item = KCFloatingActionButtonItem()
        itemDefaultSet(item)
        item.title = title
    ...
    */
    ...
    fileprivate func isTouched(touches: Set<UITouch>) -> Bool {
    ...
    internal func keyboardWillShow(_ notification: Notification) {
        ...
        UIView.animate(withDuration: 0.2, delay: 0, options: UIViewAnimationOptions(), animations: {
            self.frame = CGRect(
                ...
                y: UIScreen.main.bounds.height-self.size - keyboardSize.height - self.paddingY,
                width: self.size,
                height: self.size
            )
            }, completion: nil)
